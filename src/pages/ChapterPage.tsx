...
import { chapterMeta, combinedMeta, gamesForGroup, type ChapterGroup } from '../data/gamesList';
...
export default function ChapterPage() {
  ...
  if (group === '4') chapterGroup = 4;
  else if (group === '5') chapterGroup = 5;
  else if (group === 'combined') chapterGroup = 'combined';

  if (!chapterGroup) return <Navigate to="/" replace />;

  const meta = chapterGroup === 'combined' ? combinedMeta : chapterMeta[chapterGroup];
  const games = gamesForGroup(chapterGroup);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-12 pt-6 sm:px-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {games.map((game) => (
          <GameCard key={game.id} game={game} result={scores[game.id]} />
        ))}
      </div>
    </div>
  );
